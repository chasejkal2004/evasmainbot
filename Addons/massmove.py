...
CONTROL_PANEL_ID = 938673742886867027
EMOJI = '🟢'
...
class MassMove(commands.Cog):
    ...
    @commands.command()
    async def mch(self, ctx,dst_name : str,*role_names):
        ''' "Move-to-Channel" : .mch (DEST CHANNEL) [ROLES TO MOVE] -  Moves everyone from your current channel to DEST. If roles are included, only users with the specified roles will be moved '''
        server = ctx.message.guild
        src_name = ctx.author.voice.channel.name
        src_channel = self._get_channel(server, src_name)
        dst_channel = self._get_channel(server, dst_name)
